...
function cambios() {   
    ...
    $('.pedidoUpdate').on('click', function(){  
        ...
        if(estado == "Sin empacar" || estado == "Empacado" || estado == "No empacado"){
            ...
            for (var i = 0; i < 6; i++) {
               datosOrdenados[i] = new Array(3);//Aquí se hace el arreglo para cargar las prendas 
            }
            //alert(datosArray.length);
            for(var i=0;i<datosArray.length;i++){
                var arrrayItem = datosArray[i].split("°");
                //alert(arrrayItem[4]+" "+arrrayItem[2]);
                if(arrrayItem[4] == "Sin empacar" || arrrayItem[4] == "Empacado" || arrrayItem[4] == "No empacado"){
                    for(var j=0;j<6;j++){
