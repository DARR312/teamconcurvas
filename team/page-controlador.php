...
function ordenescambio($valor,$valor2,$valor3,$valor4,$valor5){
    ...
    /*if($valor != "0"){ 
        $todas = "";
        $cambiosTodos = $wpdb->get_results( "SELECT cambio_id, fecha_creada, datos_cliente, pedido, excedente, notas, fecha_entrega, estado, vendedor_id, usuario_id FROM con_t_ventas  WHERE venta_id =".$valor." ", ARRAY_A  );
        //print_r($ventasTodas);
        if($ventasTodas){
            foreach ($cambiosTodos as $v1) {
...
function codigosprendasjson($valor,$valor2,$valor3,$valor4){
    ...
    if($valor != "0"){ 
        ...
        $todas = "";
        $codigos = $wpdb->get_results( "SELECT codigo, estado, cual, complemento_estado, fecha_cambio, referencia_id FROM con_t_trprendas WHERE 1", ARRAY_A  );
        echo json_encode($codigos);
    }
}
