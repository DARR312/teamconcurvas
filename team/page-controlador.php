    ...
    if($valor != "0"){ 
        ...
        if($valor4 == "Empacado" || $valor4 == "Despachado"){
            ...
            if($valor2 == 10){
                ...
                $codigos = $wpdb->get_results( "SELECT codigoshow, estado, cual, complemento_estado, fecha_cambio, descripcion FROM con_t_trprendas WHERE (fecha_cambio < '".$obtenidosArray[0]['fecha']." ') AND (estado != 'En satélite') AND (estado != 'Venta mayorista')  AND (estado != 'Madrugón') AND (estado != 'Venta madrugón')  AND (estado != 'Entregado')  AND (estado != 'Venta local')  AND (estado != 'Cambio plaza')  AND (estado != 'Por revisar')  AND (estado != 'Dado de baja')  AND (estado != 'Dada de baja')  AND (estado != 'Perdida')  AND (estado != 'De baja')  AND (estado != 'Robada')  AND (estado != 'Eliminada')  AND (estado != 'Venta local')  AND (estado != 'Revisado auditoria')   ORDER BY fecha_cambio ASC", ARRAY_A  );
            }else{
                $codigos = $wpdb->get_results( "SELECT codigoshow, estado, cual, complemento_estado, fecha_cambio, descripcion FROM con_t_trprendas WHERE (fecha_cambio < '".$obtenidosArray[0]['fecha']."') AND (cual = '".$valor3."')  AND (estado != 'En satélite')  AND (estado != 'Venta mayorista')  AND (estado != 'Madrugón')  AND (estado != 'Venta madrugón') AND (estado != 'Entregado') AND (estado != 'Venta local') AND (estado != 'Cambio plaza')  AND (estado != 'Por revisar')  AND (estado != 'Dado de baja')  AND (estado != 'Dada de baja')  AND (estado != 'Perdida')  AND (estado != 'De baja')  AND (estado != 'Robada')  AND (estado != 'Eliminada')  AND (estado != 'Venta local')  AND (estado != 'Revisado auditoria')   ", ARRAY_A  );
            }        
        }  
